...
class IOChannel { public:
	...
	void send_halfblock(const halfblock* data, size_t nblock) {
		send_data(data, nblock*sizeof(halfblock));
	}

	void recv_halfblock(halfblock* data, size_t nblock) {
		recv_data(data, nblock*sizeof(emp::halfblock));
	}
