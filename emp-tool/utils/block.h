...
using block = __m128i;
using halfblock = uint64_t;


// bool × bool -> bool

inline void gf2MatMul_bool(
    bool* res,          // 输出矩阵 res[m × p]
    const bool* A,      // 输入矩阵 A[m × n]
    const bool* B,      // 输入矩阵 B[n × p]
    int m, int n, int p
) {
    for (int i = 0; i < m; ++i) {
        const bool* A_row = A + i * n;
        bool* res_row = res + i * p;
        ...
        for (int j = 0; j < p; ++j) {
            ...
            for (int k = 0; k < n; ++k, ++a, b += p) {
                sum ^= (*a) & (*b); // GF(2): 异或累加 AND
            }
...
inline void gf2MatMul_bool_halfblock(
    halfblock* res,         // 输出矩阵 res[m × p]
    const bool* A,      // 输入布尔矩阵 A[m × n]
    const halfblock* B,     // 输入 block 矩阵 B[n × p]
    int m, int n, int p
) {
    for (int i = 0; i < m; ++i) {
        const bool* A_row = A + i * n;
        halfblock* res_row = res + i * p;

        for (int j = 0; j < p; ++j) {
            const bool* a = A_row;
            const halfblock* b = B + j;
            halfblock sum =  0;
...
inline void splitBlock(block b, halfblock& lo, halfblock& hi) {
    alignas(16) halfblock tmp[2];
    _mm_store_si128((__m128i*)tmp, b); // 将 block 拆到 tmp 中
    hi = tmp[0]; // 低位部分
    lo = tmp[1]; // 高位部分
}

inline halfblock LBlock(block b) {
    return _mm_extract_epi64(b, 1); // index = 1 表示取高64位
}
...
inline bool getMSB(const block & x) {
	return (((uint8_t*)&x)[15] >> 7) & 1;
}
