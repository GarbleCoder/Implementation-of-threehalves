...
int main(void) {
	...
	for(int i = 0; i < 1024*1024*20; ++i) {
		...
		w0 = halfgates_garble(data[0], data[0]^delta, data[1], data[1]^delta, delta, table, &mi_gen);
		w1 = w0 ^ delta;
		// tregates_garble(data[0], data[0]^delta, data[1], data[1]^delta, delta, halftable, z, &mi_gen);
		data1[0] = data[0] ^ delta;
		data1[1] = data[1] ^ delta;
		ret = halfgates_eval(data1[0], data1[1], table, &mi_eva);
	}
	cout << 1024*1024*20/(time_from(start))*1e6 << " gates/second(Garble)" << endl;
	...
	// 	ret = halfgates_eval(data1[0], data1[1], table, &mi_eva);
	// }
	// cout << 1024*1024*20/(time_from(start))*1e6 << " gates/second(evaluator)" << endl;
